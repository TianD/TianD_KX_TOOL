...
def exportMCCache(geo,cacheFile,startFrame=1,endFrame=100,useTimeline=True,filePerFrame=False,cachePerGeo=True,forceExport=False):
	...
	cacheDir, fileName = os.path.split(cacheFile)
	baseName, fileExt = os.path.split(fileName)
...
def exportAbcCache(geo, cacheFile, startFrame=1, endFrame=100, useTimeline=True, verbose = True):
	'''
	@param geo: List of geometry to cache
	@type geo: List
	@eg geo: ['geometry1', 'geometry2']
	@param cacheFile: Output file name
	@type cacheFile: str
	@eg cacheFile: 'e:\\proj\\cache\\alembic\\sceneName\\example.abc'
	@param startFrame: Start frame for cache output
	@type startFrame: int
	@eg startFrame: 1
	@param endFrame: End frame for cache output
	...
	@type useTimeline: bool
	@eg useTimeline: True
	@param verbose: Set whether display progress
	@type verbose: bool
	@eg verbose: True
	'''
	...
	alembic_job_args = [
					...
					'-writeVisibility',
					'-eulerFilter',
					'-file {0}'.format(cacheFile)
					]
	
	for i in geo:
		rootStr = '-root {0}'.format(i)
		alembic_job_args.insert(-2, rootStr)
	
	mc.AbcExport(verbose = verbose, jobArg = ' '.join(alembic_job_args))
